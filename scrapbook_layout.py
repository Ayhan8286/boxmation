...
def make_cluster(photo_list, float_dir):
    margin = "ml-10 mb-10 mr-auto md:mr-0" if float_dir == 'right' else "mr-10 mb-10 ml-auto md:ml-0"
    html = f'<div class="relative float-none md:float-{float_dir} {margin} w-72 md:w-96 h-80 md:h-96 my-8 md:my-0 z-20 flex justify-center items-center">'
    
    rotations = ['-rotate-6', 'rotate-3', 'rotate-12']
    z_indexes = ['z-10', 'z-20', 'z-30']
    offsets = ['-ml-8 mt-4', 'ml-0 -mt-8', 'ml-12 mt-12']
    
    for i, p in enumerate(photo_list):
        r = rotations[i % len(rotations)]
        z = z_indexes[i % len(z_indexes)]
        off = offsets[i % len(offsets)]
        
        html += f"""
        <div class="absolute {off} {z} w-48 md:w-56 {r} transition-all duration-500 hover:rotate-0 hover:scale-110 hover:z-50 cursor-pointer">
            <div class="bg-surface-container-highest p-2 pb-6 rounded-xl border border-outline-variant/30 shadow-2xl">
              ...
              <div class="absolute top-2 left-1/2 -translate-x-1/2 w-6 h-1.5 bg-surface-bright/50 backdrop-blur-md rounded-full shadow-sm -rotate-2"></div>
            </div>
        </div>
        """
    html += '</div>'
...
print("Scrapbook story layout injected!")
